...
export async function getMatchPermissions(userId) {
    const response = await fetch(process.env.NEXT_PUBLIC_API_MATCH_PERMISSIONS, {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({id: userId})
    });
...
export async function resetChoices(userId) {
    const response = await fetch(process.env.NEXT_PUBLIC_API_CHOICE_RESET, {
        method: "POST",
        headers: {"Content-Type": "application/json"},
